...
export const buyYesNo = (market_id, size_usd, side = 'yes', mode = 'live', confirm = true) =>
  client.post('/api/predictions/trade', { market_id, size_usd, side, mode, confirm }).then(unwrap);
export const demoTrade = (market_id, size_usd, side = 'yes') => client.post('/api/predictions/demo-trade', { market_id, size_usd, side }).then(unwrap);
